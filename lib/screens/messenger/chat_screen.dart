...
class _Chat_ScreenState extends State<Chat_Screen> {
  ...
  void queryRowCount() async {
    ...
    final allRows = await dbHelper.queryRowCount(
        widget.rtmpeerid.toString(), userpeerid.toString());
    widget.logController.clear();
    allRows.forEach((row) => {
          if (row != null)
            {
              widget.logController.addLog(row["message"] +
                  "#@####@#" +
                  row["diraction"] +
                  "#@####@#" +
                  row['type1'] +
...
/*    floatingActionButton:  Padding(
                                      ...
                                      children: [
                                        Container(
                                          ...
                                          child: ClipRRect(
                                            borderRadius:
                                                BorderRadius.circular(10),
                                            child: widget.recentchatuserdetails.profile_image == ""
                                                ? Image.asset(
                                      ...
                                      Navigator.of(context).push(
                                        MaterialPageRoute(
                                          builder: (context) => TestLiveStream(
                                              ...
                                              userCallId: widget
                                                  .recentchatuserdetails.id.toString(),
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              children: [
                                                                                ...
                                                                                SizedBox(width: 1.w),
                                                                                if (parts[3] != 'Receive')
                                                                                  doubletick(parts, parts[2], parts[6],log[i])
                                                                              ],
                                                                            ),
                                                                          ),
                                                                        )
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              child: Row(
                                                                                ...
                                                                                children: [
                                                                                  ...
                                                                                  if (parts[3] != 'Receive') doubletick(parts, parts[2], parts[6],log[i])
      ...
      onWillPop: () async {
        dbHelper.deletebadge(widget.rtmpeerid);
        return Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const BottomNavbar(index: 2)),
            (Route<dynamic> route) => false);
      },
    );
  }
  ...
  Future<void> getImage(ImageSource source) async {
    ...
    if (replytex != "") {
     ...
     // _insert(replytex + "#@####@#replay#@####@#" + pickedFile.path, 'image', 'send', widget.rtmpeerid.toString());
      ...
      var file = replytex + "#@####@#noreplay#@####@#" + pickedFile.path + '#@####@#send' + '#@####@#image' + "#@####@#" + DateTime.now().toString() + "#@####@#" + "" + "#@####@#" + widget.rtmpeerid + "#@####@#" + "new";
      widget.logController.addLog(file);
  ...
  _pickpdf() async {
    ...
    if (replytex != "") {
      var file = replytex + "#@####@#replay#@####@#" + files.path + "#@#&" + files.name + '#@####@#send' + '#@####@#doc' + "#@####@#" + DateTime.now().toString() + "#@####@#" + "" + "#@####@#" + widget.rtmpeerid + "#@####@#" + "new";
      widget.logController.addLog(file);
      _insert(replytex + "#@####@#replay#@####@#" + files.path + "#@#&" + files.name, 'doc', 'send', widget.rtmpeerid.toString());
    } else {
      var file = replytex + "#@####@#noreplay#@####@#" + files.path + "#@#&" + files.name + '#@####@#send' + '#@####@#doc' + "#@####@#" + DateTime.now().toString() + "#@####@#" + "" + "#@####@#" + widget.rtmpeerid + "#@####@#" + "new";
      widget.logController.addLog(file);
  ...
  void _sendPeerMessage() async {
    print("out_message"+_peerMessage.text);
    ...
    try {
      if (replytex != "") {
        ...
        _peerMessage.clear();
        setState(() {
          replyvisibility = false;replytex = "";
        });
      } else {
        ...
        if(_peerMessage.text.trim()!="") {
          widget.logController.addLog(
              replytex + '#@####@#noreplay#@####@#' + _peerMessage.text +
                  '#@####@#send' + '#@####@#text' + '#@####@#' +
                  DateTime.now().toString() + '#@####@#' + id.toString() +
                  "#@####@#" + widget.rtmpeerid + "#@####@#" + "new");
        } _peerMessage.clear();
      }
    } catch (errorCode) {
      ...
      if (replytex != "") {
        print("messagereplayelse"+_peerMessage.text);
        AgoraRtmMessage message = AgoraRtmMessage.fromText(replytex +
            "#@####@#replay#@####@#" +
            _peerMessage.text +
            "#@####@#" +
  ...
  Future<String> _popupdialog(
      type, String selectedmsgid, String message, int i) async {
    var hidecopy = false;
    dynamic data_list = [];
  ...
  imageprogressbar(total, current, i) {
    ...
    else if(uplordstatus == 2){
      print('here error else');
      return const Icon(
        Icons.error,
        color: Colors.redAccent,
        size: 40,
  ...
  void rtmforwordmessage(forwordedmessage, joiner) {
    ...
    if (ext == "jpg" || ext == "jpeg" || ext == "png") {
      var file = replytex +
          ...
          joiner.peerId;
      widget.logController.addLog(file);
      _insert(replytex + "#@####@#replay#@####@#" + forwordedmessage, 'image',
          'send', joiner.peerId);
      ...
      widget.logController.addLog(file);
      _insert(
          replytex + "#@####@#noreplay#@####@#" + name[0] + "#@#&" + name[1],
          'doc',
          'send',
          joiner.peerId);
      ...
      widget.logController.addLog(file);
      _insert(
          replytex + "#@####@#noreplay#@####@#" + name[2] + "#@#&" + name[1],
          'video',
          'send',
          joiner.peerId);
      AgoraRtmMessage message = AgoraRtmMessage.fromText(
          "video#@####@#noreplay#@####@#" + name[2] + "#@#&" + name[1]);
      ...
      AgoraRtmMessage message = AgoraRtmMessage.fromText(replytex +
          ...
          DateTime.now().toString());
      widget.client.sendMessageToPeer(joiner.peerId, message, true, false);
      var id = _insert(replytex + "#@####@#forword#@####@#" + forwordedmessage,
          "text", 'send', joiner.peerId);
  ...
  doubletick(parts, msg, id,logindex) {
   // print("partslength" + parts.length.toString());
    ...
    else if (( onlinestatus == "Offline" && parts[8] == "new") || parts[8] == "Undelivered" ) {
   ...
   statusWidget() {
    var status="";
    if(widget.status!="inactive"||widget.status==null||widget.recentchatuserdetails.transit_allowed!="No"){
      status=onlinestatus;
    } else if(widget.recentchatuserdetails.transit_allowed=="No"&& widget.status=="active") {
